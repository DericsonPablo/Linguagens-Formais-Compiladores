...
# Disciplina de Linguagens Formais e Compiladores
# Professor José Carlos
# Alunos: André Bernardo RA: 16399926
#		  Dericson Pablo RA: 16399426
#		  Gabriel Henrique Simioni RA: 16417426
#		  Railan Dal Col RA: 16095326
# ==============================================================================
...
# Identificador Mal digitado
# Tamanho do identificador
# Numero mal digitado
# Fim de arquivo não esperado (comentario de bloco que nao foi fechado)
# Caractere ou string mal digitadas
# ==============================================================================
...
class AnalisadorLexico():
  ...
  # Metodo que especifica qual dos DTs delimitadores eh a entrada
  def qualDTDelimitador(self, entrada):
    # String com os operadores componentes da linguagem
    delimitadores = ";,(){}[]"
    posicao = delimitadores.find(entrada)
    return "[Delimitador] - DT: "+str(posicao)
  ...
  # Metodo que especifica qual dos DTs operadores eh a entrada
  def qualDTOperador(self, entrada):
    # Listas com os operadores componentes da linguagem
    operadores = '+ - * / == > < ='.split()
    posicao = 0
    for x in operadores:
      ...
      posicao += 1
    if(posicao == 0):
      return "[Operador +] - DT: 8 "
    elif(posicao == 1):
      return "[Operador -] - DT: 9 "
    elif(posicao == 2):
      return "[Operador *] - DT: 11 "
    elif(posicao == 3):
      return "[Operador /] - DT: 10 "
    elif(posicao == 4):
      return "[Operador ==] - DT: 12 "
    elif(posicao == 5):
      return "[Operador >] - DT: 13 "
    elif(posicao == 6):
      return "[Operador <] - DT: 14 "
    elif(posicao == 7):
      return "[Operador =] - DT: 15 "
  ...
  def ehReservada(self, entrada):
    ...
    reservadas = "if else read write void int main Main while".split()
    if entrada in reservadas:
      return True
    return False

  # Metodo que especifica qual dos DTs palavras reservadas eh a entrada
  def qualDTReservada(self, entrada):
    # Listas com os operadores componentes da linguagem
    reservadas = '''if else read write void int main Main while'''.split()
    posicao = 0
    for x in reservadas:
      ...
      posicao += 1
    if(posicao > 9):
      return "[Palavra Reservada] - DT:"+str(posicao)
    else:
      return ""
    ...
    while linha_programa:
      ...
      while i < tamanho_linha: #Percorre os caracteres da linha
        ...
        if (self.ehDelimitador(caracter_atual)):
          arquivo_saida.write(self.qualDTDelimitador(caracter_atual)+' - Delimitador encontrado: '+caracter_atual+' - linha de localização->'+str(numero_linha)+'\n')
        # ===================================================================================
        ...
        # Verificando se o elemento eh um operador
          ...
          arquivo_saida.write(self.qualDTOperador(caracter_atual+caractere_seguinte)+'- linha de localização->'+str(numero_linha)+'\n')
          i += 1
        elif self.ehOperador(caracter_atual):
          arquivo_saida.write(self.qualDTOperador(caracter_atual)+'- linha de localização->'+str(numero_linha)+'\n')
        ...
        # string.punctuation[6] retorna o simbolo - ' - que representa o inicio do caractere constante
          ...
          if (linha_programa[i+1] == '\n') or (not (string.punctuation[6] in linha_programa[i+1:])):
            ...
            arquivo_saida.write('[Caractere Constante '+linha_programa[i+1]+'] - linha de localização->'+str(numero_linha)+'\n')
            i+=2
          ...
          if (linha_programa[i:].find(string.punctuation[1]) == -1):
            ...
            for x in nova_cadeia:
              if(not self.ehSimbolo(x)):
                ...
                break
            if(ehValido):
              arquivo_saida.write('[String '+nova_cadeia+'] DT = 2 - linha de localização->'+str(numero_linha)+'\n')
        # ===================================================================================
        # Verificando se o elemento em questao eh um numero - OK
          ...
          if (caracter_atual == '.'):
            ...
            if (j > 0):
              arquivo_saida.write('[Número Real'+string_temp+'] - linha de localização:->'+str(numero_linha)+'\n')
            else: 
              arquivo_saida.write('Erro Lexico - Numero mal formado - Linha: %d\n' %numero_linha)
          else:
            arquivo_saida.write('[Número inteiro '+string_temp+'] DT = 300  - linha de localização:->'+str(numero_linha)+'\n')
          ...
          if (algum_erro):
            ...
            if (self.ehReservada(string_temp)):
              ...
              if(string_temp == "if"):
                ...
                token = " DT = 7 "
              arquivo_saida.write(self.qualDTReservada(string_temp)+'[Palavra Reservada '+string_temp+']'+token+'- linha de localização->'+str(numero_linha)+'\n')
            else:
              arquivo_saida.write('[Identificador '+string_temp+'] - DT: 1 - linha de localização->'+str(numero_linha)+'\n')
