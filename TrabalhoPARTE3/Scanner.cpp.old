...
void Scanner::Init() {
	...
	noSym = 27;
	int i;
	for (i = 97; i <= 100; ++i) start.set(i, 1);
	for (i = 102; i <= 114; ++i) start.set(i, 1);
	...
	keywords.set(L"begin", 3);
	keywords.set(L"input", 11);
	keywords.set(L"print", 12);
	keywords.set(L"do", 23);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 1:
			...
			if ((ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
		...
		case 28:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'm') || (ch >= L'o' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'n') {AddCh(); goto case_32;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 29:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'g') || (ch >= L'i' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'h') {AddCh(); goto case_33;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 30:
		...
		case 32:
			case_32:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'c') || (ch >= L'e' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'd') {AddCh(); goto case_34;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 33:
			case_33:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'n') || (ch >= L'p' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'o') {AddCh(); goto case_35;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 34:
			case_34:
			recEnd = pos; recKind = 1;
		...
		case 35:
			case_35:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'v') || (ch >= L'x' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'w') {AddCh(); goto case_36;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 36:
			case_36:
			recEnd = pos; recKind = 1;
			if ((ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'S') {AddCh(); goto case_37;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 37:
			case_37:
			if (ch == L'y') {AddCh(); goto case_38;}
			else {goto case_0;}
		case 38:
			case_38:
			if (ch == L'm') {AddCh(); goto case_39;}
			else {goto case_0;}
		case 39:
			case_39:
			if (ch == L'b') {AddCh(); goto case_40;}
			else {goto case_0;}
		case 40:
			case_40:
		...
		case 43:
			...
			else if (ch == L'r') {AddCh(); goto case_11;}
			else {goto case_0;}
